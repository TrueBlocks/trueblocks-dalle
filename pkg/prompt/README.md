...
Literary enhancement additionally omits temperature for gpt-5 models and appends
the enhance instruction, filled from the registry, to the system context.
Neither path adds a token limit or an additional retry loop; the shared client's
default single attempt preserves existing behavior.
